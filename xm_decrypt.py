...
def main(input_path, output_path):
    ...
    if choice == "1" or choice == "2":
        if choice == "1":
            ...
            decrypt_xm_file(files_to_decrypt, output_path)
            if files_to_decrypt == [""]:
                print("检测到文件选择窗口被关闭")

    #     elif choice == "2":
    #         # dir_to_decrypt = input('输入路径')
    #         dir_to_decrypt = input_path
    #         if dir_to_decrypt == "":
    #             print("检测到目录选择窗口被关闭")
    #
    #         files_to_decrypt = dir_to_decrypt
    #         # print("请选择是否需要设置输出路径：（不设置默认为本程序目录下的output文件夹）")
    #         # print("1. 设置输出路径")
    #         # print("2. 不设置输出路径")
    #         choice = '1'
    #         if choice == "1":
    #             # output_path = input('输入路径')
    ...
    #         else:
    #             output_path_ = "./output"
    #         for root, dirs, files in os.walk(files_to_decrypt):
    #             for file in files:
    #                 if file.endswith(".xm"):
    #                     decrypt_xm_file(dir_to_decrypt + r'\\' + file, output_path_)
    #         else:
    #             pass
    # elif choice == "3":
    #     sys.exit()
...
if __name__ == "__main__":
    input_path_ = r"C:\Users\21385\Documents\WeChat Files\wxid_fcddel06m7j422\FileStorage\File\2024-10\总有这样的歌只想一个人听(1)\总有这样的歌只想一个人听"
    for root, dirs, files in os.walk(input_path_):
        for file in files:
            if file.endswith(".xm"):
                decrypt_xm_file(input_path_ + r'\\' + file, './output')
    # main('./input', './output')
